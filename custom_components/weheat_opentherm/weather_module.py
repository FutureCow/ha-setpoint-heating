...
# Hoeveel graden de aanvoertemperatuur omlaag gaat per conditie (zonne-opbrengst)
_SUN_CORRECTION_BY_CONDITION: dict[str, float] = {
    "sunny": 3.0,
    "partlycloudy": 2.0,
    "windy-variant": 1.0,
}
...
def _sun_correction(condition: str, wind_kmh: float) -> float:
    """Opwarmreductie door zonne-instaling. Nul bij te hoge windsnelheid."""
    if wind_kmh >= _SUN_WIND_LIMIT:
        return 0.0
    return _SUN_CORRECTION_BY_CONDITION.get(condition, 0.0)
...
async def async_get_forecast_corrections(
    ...
    for entry in forecasts[:hours]:
        ...
        sun_values.append(_sun_correction(condition, wind))
